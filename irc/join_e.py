    ...
    if ( len(records) == 0 ):   # user NOT found, add him
        ...
        if ( len(records) != 0 ):    # he has messages in database, read it
            ...
            for msg in records:
                ...
                self.send_message_to_channel( ("%s, offline msg from `%s` at `%s %s UTC`: %s" % (irc_join_nick, msg[0], date_l, time_l, msg[3])), msg[1])  # send offline message to channel where it was created
                time.sleep(2)
