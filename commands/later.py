...
def later(self, user, channel):
    ...
    user_nicks_list = self.get_names(channel)
    for user_nick in user_nicknames:
        ...
        if user_nick.lower() in user_nicks_list:  # reciever is on the channel right now
            self.send_reply( ("Error! "+user_nick+" is online!"), user, channel )
            continue
        # reciever is not on the channel; check if he exists in database
